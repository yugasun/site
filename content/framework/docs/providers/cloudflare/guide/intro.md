...
layout: Doc
gitLink: /docs/providers/cloudflare/guide/intro.md
---
...
### Events
Anything that triggers a Cloudflare Worker Event to execute is regarded by the Framework as an **Event**. The only event that triggers a Cloudflare Worker is an HTTP request. Since the only event that can trigger a Worker is an HTTP request, declaring events is optional, and only used to declare specific endpoints that can be called by [`serverless invoke`](../../cli-reference/invoke). This is useful for defining specific hooks into your application for testing.
