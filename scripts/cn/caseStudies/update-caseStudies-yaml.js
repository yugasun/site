...
const replacePattern = /[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-9]|[1-2][0-9]|3[0-1])-/g
...
module.exports = function updateFileContents(filePath, callBack) {
  dir.readFiles(filePath, {
    ...
    const filenameBase = path.parse(filename).base;
    const newName = filenameBase.replace(replacePattern, '').toLowerCase();
    item.gitLink = `/${newName}`
    if (item.date) {
