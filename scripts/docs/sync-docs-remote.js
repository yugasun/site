...
rimraf(config.serverlessRepoPath, () => {
  ...
  const latestReleaseUrl = config.downloadLink || getLatestReleaseUrl();
...
const getLatestReleaseUrl = () => {
    ...
    const res = request('GET', 'https://api.github.com/repos/serverless/serverless/releases/latest', opts)
