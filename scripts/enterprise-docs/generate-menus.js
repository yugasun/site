...
function getDirectories() {
  const docsPath = config.siteEnterpriseDocsPath + '/**'
  const not = '!**/*.*'
  ...
  return getDirectories()
    .then(directories => {
      ...
      directories.forEach(dir => {
        ...
        menuData[cleanDirName].index = files
          .filter(f => {
        ...
        menuData[cleanDirName].children = files
          .filter(f => {
            return !f.match('index')
          })
          .map(f => {
...
function generateDocMenu() {
  buildMenuObject()
    .then(data => {
      ...
      const fileName = 'generated-enterprise-menu-items.js'
