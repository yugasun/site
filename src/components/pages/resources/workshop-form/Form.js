...
const RadioButtonWithLabel = ({ id, name, text, onChange }) => (
  <Label htmlFor={id}>
    <Flex.verticallyCenter my='8px'>
      <RadioButton name={name} id={id} onChange={onChange} required />
      <InlineBlock>{text}</InlineBlock>
    </Flex.verticallyCenter>
  </Label>
)
...
export default class WorkshopForm extends React.Component {
  ...
  render() {
    ...
    if (success) {
      return (
        <AppContainer>
          <Flex.center mt={5} mb={[300, 300, 14]}>
            <Box width={[1, 1, 1, 0.8]}>
              <Paragraph align='center'>
                Great! You’re now on the waitlist for one of our workshops. If
                you’re new to Serverless, why not{' '}
                <a href='https://serverless.com/framework/docs/'>
                  check out our documentation
                </a>{' '}
                to get started, or head on{' '}
                <a href='https://serverless.com/blog/'>over to our blog?</a>
              </Paragraph>
            </Box>
          </Flex.center>
        </AppContainer>
        ...
        <AppContainer>
          <Box mt={5} mb={[92, 92, 32, 32, 92]}>
            <Row>
              ...
              >
                <FormHeading fontFamily='SoleilBk' lineHeight={3}>
                  Contact Information
                </FormHeading>
              </TextWithIcon>
            </Row>
            <ResponsiveStack justifyContent='space-between' py={1}>
            ...
            <ResponsiveStack justifyContent='space-between' py={1}>
              <Column width={[1, 1, 3 / 10]} py={1}>
                ...
                <Box>
                  ...
                  <RadioButtonWithLabel
                    ...
                    onChange={() =>
                      this.setState({ experience: 'professional' })
                    }
                  />
                </Box>
              </Column>
              <Column width={[1, 1, 3 / 10]} py={1}>
                ...
                <Box>
                  {['aws', 'google_cloud_platform', 'microsoft_azure'].map(
                    poi => (
                      <CheckboxWithLabel
                        id={poi}
                        key={poi}
                        ...
                        onChange={({ target }) =>
                          this.updateCheckbox('poi', poi, target.value)
                        }
                      />
                    )
                  )}
              ...
              <Column width={[1, 1, 3 / 10]} py={1}>
                ...
                <Box>
                  {['node_js', 'python', 'java', 'ruby', 'php', 'go'].map(
                    lang => (
                      <CheckboxWithLabel
                        id={lang}
                        key={lang}
                        ...
                        onChange={({ target }) =>
                          this.updateCheckbox('language', lang, target.value)
                        }
                      />
                    )
                  )}
            ...
            <ResponsiveStack justifyContent='space-between' py={1}>
              <Column width={1}>
                ...
                <TextArea
                  ...
                  onChange={({ target }) =>
                    this.setState({ serverless_goals: target.value })
                  }
                  width={1}
                />
              </Column>
            ...
            <Row>
              <Column mt={5} alignItems='center' width={1}>
