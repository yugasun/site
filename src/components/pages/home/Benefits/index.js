...
const MobileBenefitItem = ({ number, title }) => (
  <Box display={['block', 'block', 'none']} mt={15} mb={25}>
    <Row>
      <Button
        bg='#fd5750'
        width={['60px', '60px', '72px']}
        height={['60px', '60px', '72px']}
        fontSize={['26.7px', '26.7px', '32px']}
        p={0}
      >
        {number}
      </Button>

      <P color='black' fontSize='24px' ml={'32px'} mt={15}>
        {title}
      </P>
    </Row>
  </Box>
...
const MobileBenefitItemTablet = ({ number, title, width }) => (
  <Box display={['none', 'none', 'block', 'block', 'none']}>
    <Row>
      <Button
        bg='#fd5750'
        width={['60px', '60px', '43.6px', '43.6px', '72px']}
        height={['60px', '60px', '43.6px', '43.6px', '72px']}
        fontSize={['26.7px', '26.7px', '26.7px', '26.7px', '32px']}
        p={0}
      >
        {number}
      </Button>
...
export default class HomeBenefits extends React.Component {
  ...
  componentDidMount() {
    ...
    if (windowGlobal) {
      if (windowGlobal.innerWidth <= 1279) {
        this.setState({ isUsingMobile: true })
      }

      if (windowGlobal.innerWidth >= 1600) {
        this.setState({
          benefitsAnimationStyles: BenefitsAnimationStylesDesktop,
        })
      } else {
        this.setState({
          benefitsAnimationStyles: BenefitsAnimationStylesLaptop,
        })
      }
    }
  }
  ...
  onScroll = () => {
    ...
    const scrollMovement =
      this.state.stopAnimation || currentScrollY > this.state.lastScrollY
        ? 'down'
        : 'up'

    this.setState({
      currentScrollMovement: scrollMovement,
    ...
    if (!this.state.isUsingMobile) {
      if (
        this.state.currentScrollMovement === 'down' &&
  ...
  render() {
    return (
      <AppContainer>
        <Box
          style={
            this.state.benefitsAnimationTriggered
              ? BenefitsContainerAnimationStyles
              : {}
          }
        >
          <DesktopDownArrow
            animationTriggered={
              this.state.benefitsAnimationTriggered ? true : false
            }
          />
        </Box>
        ...
        <ResponsiveStack.spaceBetween
          ...
          mt={[100, 100, 69, 69, 100]}
          mb={[50, 50, 29, 29, 200]}
          style={
            this.state.benefitsAnimationTriggered
              ? this.state.benefitsAnimationStyles
              : {}
          }
        >
          ...
          <MobileCenteredImage
            ...
            style={{
              display:
                this.state.isUsingMobile || this.state.showDataProcessing
                  ? 'block'
                  : 'none',
            }}
            width={[250, 250, 181, 181, 515, 615]}
            height={[233, 233, 169, 169, 581]}
          />

          <MobileBenefitItem number='02' title='Custom automation' />
          <MobileCenteredImage
            src={customAutomation}
            style={{
              display:
                this.state.isUsingMobile || this.state.showCustomAutomation
                  ? 'block'
                  : 'none',
            }}
            width={[250, 250, 181, 181, 515, 615]}
          ...
          <MobileCenteredImage
            ...
            style={{
              display:
                this.state.isUsingMobile || this.state.showBuildApi
                  ? 'block'
                  : 'none',
            }}
            width={[250, 250, 181, 181, 581]}
            height={[233, 233, 169, 169, 481, 581]}
          />
          ...
          <Box display={['block', 'block', 'none', 'none', 'block']}>
            <Column width={[1, 1, '488px', '488px', '368px', '488px']}>
              ...
              <Box display={['none', 'none', 'none', 'none', 'block']}>
                <Row
                  onClick={() => this.showBenefit('buildApi')}
                  mt={[0, 0, 40]}
                >
                  <Button
                    bg={this.state.showBuildApi ? '#fd5750' : '#eaeaea'}
                    width={['60px', '60px', '72px']}
                    height={['60px', '60px', '72px']}
                    fontSize='32px'
                  >
                ...
                <Row
                  onClick={() => this.showBenefit('customAutomation')}
                  mt={[0, 0, 25]}
                >
                  <Button
                    bg={this.state.showCustomAutomation ? '#fd5750' : '#eaeaea'}
                    width={['60px', '60px', '72px']}
                    height={['60px', '60px', '72px']}
                    fontSize='32px'
                  >
                ...
                <Row
                  onClick={() => this.showBenefit('dataProcessing')}
                  mt={[0, 0, 25]}
                >
                  <Button
                    bg={this.state.showDataProcessing ? '#fd5750' : '#eaeaea'}
                    width={['60px', '60px', '72px']}
                    height={['60px', '60px', '72px']}
                    fontSize='32px'
                  >
