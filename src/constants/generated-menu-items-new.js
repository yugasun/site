module.exports = [
  {
    to: '/framework/docs/',
    label: 'framework open-source',
    icon: 'sls-root-menu',
    content: [
      {
        to: '/framework/docs/getting-started/',
        label: 'getting started',
      },
      {
        to: '/framework/docs/providers/',
        label: 'providers',
        content: [
          {
            to: '/framework/docs/providers/aws/',
            label: 'AWS',
            content: [
              {
                to: '/framework/docs/providers/aws/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/config-credentials/',
                    label: 'config credentials',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/install/',
                    label: 'install',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/package/',
                    label: 'package',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/deploy-function/',
                    label: 'deploy function',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/deploy-list/',
                    label: 'deploy list',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/invoke-local/',
                    label: 'invoke local',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/login/',
                    label: 'login',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/metrics/',
                    label: 'metrics',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/rollback/',
                    label: 'rollback',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/rollback-function/',
                    label: 'rollback function',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/print/',
                    label: 'print',
                  },
                  {
                    to: '/framework/docs/providers/aws/cli-reference/slstats/',
                    label: 'serverless stats',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/aws/events/',
                label: 'AWS events',
                content: [
                  {
                    to: '/framework/docs/providers/aws/events/apigateway/',
                    label: 'api gateway',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/websocket/',
                    label: 'websocket',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/streams/',
                    label: 'kinesis & dynamodb',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/s3/',
                    label: 's3',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/schedule/',
                    label: 'schedule',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/sns/',
                    label: 'sns',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/sqs/',
                    label: 'sqs',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/alexa-skill/',
                    label: 'alexa skill',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/iot/',
                    label: 'iot',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/events/cloudwatch-event/',
                    label: 'cloudwatch event',
                  },
                  {
                    to: '/framework/docs/providers/aws/events/cloudwatch-log/',
                    label: 'cloudwatch log',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/events/cognito-user-pool/',
                    label: 'cognito user pool',
                  },
                  {
                    to:
                      '/framework/docs/providers/aws/events/alexa-smart-home/',
                    label: 'alexa smart home',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/aws/examples/',
                label: 'examples',
              },
              {
                to: '/framework/docs/providers/aws/guide/',
                label: 'user guide',
                content: [
                  {
                    to: '/framework/docs/providers/aws/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/credentials/',
                    label: 'credentials',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/layers/',
                    label: 'layers',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/resources/',
                    label: 'resources',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/testing/',
                    label: 'testing',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/variables/',
                    label: 'variables',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/packaging/',
                    label: 'packaging',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/iam/',
                    label: 'iam',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/plugins/',
                    label: 'plugins',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/workflow/',
                    label: 'workflow',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/serverless.yml/',
                    label: 'serverless.yml',
                  },
                  {
                    to: '/framework/docs/providers/aws/guide/v0_to_v1/',
                    label: 'v.0 & v.1',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/azure/',
            label: 'azure',
            content: [
              {
                to: '/framework/docs/providers/azure/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to: '/framework/docs/providers/azure/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/install/',
                    label: 'install',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/deploy-function/',
                    label: 'deploy function',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/login/',
                    label: 'login',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/azure/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                  {
                    to: '/framework/docs/providers/azure/cli-reference/print/',
                    label: 'print',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/azure/events/',
                label: 'azure events',
                content: [
                  {
                    to: '/framework/docs/providers/azure/events/http/',
                    label: 'http',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/timer/',
                    label: 'timer',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/queuestorage/',
                    label: 'queue storage',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/servicebus/',
                    label: 'service bus',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/eventhubs/',
                    label: 'event hubs',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/blobstorage/',
                    label: 'blob storage',
                  },
                  {
                    to: '/framework/docs/providers/azure/events/other/',
                    label: 'other bindings',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/azure/examples/',
                label: 'examples',
              },
              {
                to: '/framework/docs/providers/azure/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/azure/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/credentials/',
                    label: 'credentials',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/testing/',
                    label: 'testing',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/variables/',
                    label: 'variables',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/packaging/',
                    label: 'packaging',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/plugins/',
                    label: 'plugins',
                  },
                  {
                    to: '/framework/docs/providers/azure/guide/workflow/',
                    label: 'workflow',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/cloudflare/',
            label: 'cloudflare',
            content: [
              {
                to: '/framework/docs/providers/cloudflare/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/cloudflare/events/',
                label: 'cloudflare workers events',
                content: [
                  {
                    to: '/framework/docs/providers/cloudflare/events/http/',
                    label: 'http events',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/cloudflare/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/cloudflare/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to:
                      '/framework/docs/providers/cloudflare/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/debugging/',
                    label: 'debugging',
                  },
                  {
                    to: '/framework/docs/providers/cloudflare/guide/workflow/',
                    label: 'workflow',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/fn/',
            label: 'fn',
            content: [
              {
                to: '/framework/docs/providers/fn/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to: '/framework/docs/providers/fn/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to: '/framework/docs/providers/fn/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to: '/framework/docs/providers/fn/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to: '/framework/docs/providers/fn/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to: '/framework/docs/providers/fn/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to: '/framework/docs/providers/fn/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/fn/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/fn/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/fn/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/fn/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/fn/events/',
                label: 'fn events',
                content: [
                  {
                    to: '/framework/docs/providers/fn/events/http/',
                    label: 'http events',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/fn/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/fn/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/debugging/',
                    label: 'debugging',
                  },
                  {
                    to: '/framework/docs/providers/fn/guide/workflow/',
                    label: 'workflow',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/google/',
            label: 'google',
            content: [
              {
                to: '/framework/docs/providers/google/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/install/',
                    label: 'install',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/package/',
                    label: 'package',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to: '/framework/docs/providers/google/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/invoke-local/',
                    label: 'invoke local',
                  },
                  {
                    to: '/framework/docs/providers/google/cli-reference/login/',
                    label: 'login',
                  },
                  {
                    to: '/framework/docs/providers/google/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/google/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                  {
                    to: '/framework/docs/providers/google/cli-reference/print/',
                    label: 'print',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/google/events/',
                label: 'google functions events',
                content: [
                  {
                    to: '/framework/docs/providers/google/events/http/',
                    label: 'http',
                  },
                  {
                    to: '/framework/docs/providers/google/events/event/',
                    label: 'event',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/google/examples/',
                label: 'examples',
              },
              {
                to: '/framework/docs/providers/google/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/google/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/credentials/',
                    label: 'credentials',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/packaging/',
                    label: 'packaging',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/variables/',
                    label: 'variables',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/plugins/',
                    label: 'plugins',
                  },
                  {
                    to: '/framework/docs/providers/google/guide/workflow/',
                    label: 'workflow',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/kubeless/',
            label: 'kubeless',
            content: [
              {
                to: '/framework/docs/providers/kubeless/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/kubeless/events/',
                label: 'kubeless events',
                content: [
                  {
                    to: '/framework/docs/providers/kubeless/events/http/',
                    label: 'http events',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/events/pubsub/',
                    label: 'pubsub',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/events/scheduler/',
                    label: 'schedule',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/kubeless/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/kubeless/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to:
                      '/framework/docs/providers/kubeless/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/events/',
                    label: 'events',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/debugging/',
                    label: 'debugging',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/workflow/',
                    label: 'workflow',
                  },
                  {
                    to: '/framework/docs/providers/kubeless/guide/packaging/',
                    label: 'packaging',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/openwhisk/',
            label: 'openwhisk',
            content: [
              {
                to: '/framework/docs/providers/openwhisk/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/config-credentials/',
                    label: 'config credentials',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/install/',
                    label: 'install',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/deploy-function/',
                    label: 'deploy function',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/invoke-local/',
                    label: 'invoke local',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/login/',
                    label: 'login',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/print/',
                    label: 'print',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/cli-reference/slstats/',
                    label: 'serverless stats',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/openwhisk/events/',
                label: 'openwhisk events',
                content: [
                  {
                    to:
                      '/framework/docs/providers/openwhisk/events/apigateway/',
                    label: 'api gateway',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/events/triggers/',
                    label: 'triggers',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/events/cloudant/',
                    label: 'cloudant',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/events/messagehub/',
                    label: 'message hub',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/events/schedule/',
                    label: 'schedule',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/openwhisk/examples/',
                label: 'examples',
              },
              {
                to: '/framework/docs/providers/openwhisk/guide/',
                label: 'guide',
                content: [
                  {
                    to: '/framework/docs/providers/openwhisk/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/guide/installation/',
                    label: 'installation',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/guide/credentials/',
                    label: 'credentials',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/services/',
                    label: 'services',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/functions/',
                    label: 'functions',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/events/',
                    label: 'events',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/guide/web-actions/',
                    label: 'web actions',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/deploying/',
                    label: 'deploying',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/testing/',
                    label: 'testing',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/variables/',
                    label: 'variables',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/packaging/',
                    label: 'packaging',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/plugins/',
                    label: 'plugins',
                  },
                  {
                    to: '/framework/docs/providers/openwhisk/guide/workflow/',
                    label: 'workflow',
                  },
                  {
                    to:
                      '/framework/docs/providers/openwhisk/guide/serverless.yml/',
                    label: 'serverless.yml',
                  },
                ],
              },
            ],
          },
          {
            to: '/framework/docs/providers/spotinst/',
            label: 'spotinst',
            content: [
              {
                to: '/framework/docs/providers/spotinst/guide/',
                label: 'user guide',
                content: [
                  {
                    to: '/framework/docs/providers/spotinst/guide/intro/',
                    label: 'intro',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/quick-start/',
                    label: 'quick start',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/guide/create-token/',
                    label: 'create token',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/credentials/',
                    label: 'credentials',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/guide/serverless.yml/',
                    label: 'serverless.yml',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/endpoints/',
                    label: 'endpoint set up',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/variables/',
                    label: 'variables',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/cors/',
                    label: 'cors',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/guide/active-versions/',
                    label: 'active versions',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/guide/IAM-roles/',
                    label: 'iam role',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/spotinst/cli-reference/',
                label: 'CLI reference',
                content: [
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/config-credentials/',
                    label: 'config credentials',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/create/',
                    label: 'create',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/deploy/',
                    label: 'deploy',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/deploy-function/',
                    label: 'deploy function',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/invoke/',
                    label: 'invoke',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/logs/',
                    label: 'logs',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/stage/',
                    label: 'stage variables',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/info/',
                    label: 'info',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/remove/',
                    label: 'remove',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/plugin-list/',
                    label: 'plugin list',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/plugin-install/',
                    label: 'plugin install',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/plugin-search/',
                    label: 'plugin search',
                  },
                  {
                    to:
                      '/framework/docs/providers/spotinst/cli-reference/plugin-uninstall/',
                    label: 'plugin uninstall',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/spotinst/events/',
                label: 'spotinst events',
                content: [
                  {
                    to: '/framework/docs/providers/spotinst/events/http/',
                    label: 'api gateway',
                  },
                  {
                    to: '/framework/docs/providers/spotinst/events/schedule/',
                    label: 'schedule',
                  },
                ],
              },
              {
                to: '/framework/docs/providers/spotinst/examples/',
                label: 'hello world example',
              },
            ],
          },
        ],
      },
      {
        to: '/examples/',
        label: 'examples',
      },
      {
        to: '/blog/category/guides-and-tutorials/',
        label: 'tutorials',
      },
    ],
  },
]
